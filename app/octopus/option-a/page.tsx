...
export default async function OctopusOptionAPage() {
  ...
  try {
    ...
    console.warn(
      "[octopus/option-a] Snapshot load failed, using static db:",
  ...
  const stockListStale = snapshotSyncedAt
    ? Date.now() - new Date(snapshotSyncedAt).getTime() >
      STOCK_LIST_STALE_DAYS * 24 * 60 * 60 * 1000
    : false;
